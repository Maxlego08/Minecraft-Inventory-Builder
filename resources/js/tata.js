...
const defaultOpts = {
    type: 'log', position: 'tr', animate: 'fade', // slide
    duration: 3000, progress: true, holding: false, closeBtn: true, onClick: null, onClose: null
}


window.text = function (title = 'Hello', text = 'Test message' + new Date().toLocaleString(), opts = {}) {
    const _opts = Object.assign({}, defaultOpts, opts, {type: 'text'})
    render(title, text, _opts)
}

window.log = function (title = 'Hello', text = 'Test message' + new Date().toLocaleString(), opts = {}) {
    const _opts = Object.assign({}, defaultOpts, opts, {type: 'log'})
    render(title, text, _opts)
}

window.tinfo = function (title = 'Hello', text = 'Test message' + new Date().toLocaleString(), opts = {}) {
    const _opts = Object.assign({}, defaultOpts, opts, {type: 'info'})
    render(title, text, _opts)
}

window.warn = function (title = 'Hello', text = 'Test message' + new Date().toLocaleString(), opts = {}) {
    const _opts = Object.assign({}, defaultOpts, opts, {type: 'warn'})
    render(title, text, _opts)
}

window.error = function (title = 'Hello', text = 'Test message' + new Date().toLocaleString(), opts = {}) {
    const _opts = Object.assign({}, defaultOpts, opts, {type: 'error'})
    render(title, text, _opts)
}

window.success = function (title = 'Hello', text = 'Test message' + new Date().toLocaleString(), opts = {}) {
    const _opts = Object.assign({}, defaultOpts, opts, {type: 'success'})
    render(title, text, _opts)
}

window.toast = function (type = 'log', title = 'Hello', text = 'Test', duration = 3000) {
    render(title, text, {
        type: type, position: 'tr', animate: 'fade', // slide
        duration: duration, progress: true, holding: false, closeBtn: true, onClick: null, onClose: null
    })
}
