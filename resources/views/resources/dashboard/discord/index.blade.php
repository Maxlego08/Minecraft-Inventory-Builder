@php use App\Models\UserRole;use Illuminate\Support\Str; @endphp
        ...
        <div class="card-body">
            ...
            @if(user()->role->isMember())
                <div class="alert alert-danger">
                    {!! __('resources.dashboard.discord.permission', ['role' => "<span class='btn-role btn-premium rounded-1 text-white'><i class='me-2 " . UserRole::ICON_PREMIUM . "'></i>Premium</span>"]) !!}
                </div>
            @else
                        ...
                        @foreach($discords as $discord)
                            <tr>
                                ...
                                <td class="d-flex justify-content-evenly align-items-center">
                                    ...
                                    <form action="{{ route('resources.dashboard.discord.delete', $discord) }}"
                                          method="POST"
                                    ...
                                    <form action="{{ route('resources.dashboard.discord.test', $discord) }}"
                                          method="POST">
                                        @csrf
                                        <button type="submit" class="btn"
                                                style="background: none; border: none; padding: 0;" aria-label="Submit"
            ...
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    ...
                    @foreach($events as $event)
                        <tr>
                            <td>{{ $event }}</td>
                            <td>{{ __("resources.dashboard.discord.documentation." . str_replace('.', '_', $event)) }}
                                .
                            </td>
                        </tr>
                    @endforeach
