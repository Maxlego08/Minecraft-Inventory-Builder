...
class BBCode
{
     ...
     * Code constructor.
     *
     * @param string|null $text The text - might include Code tags
     */
    ...
    /**
     * Set the raw text - might include Code tags
     *
    ...
    /**
     * Renders only the text without any Code tags.
     *
     * @param string $text Optional: Render the passed Code string instead of the internally stored one
     * @return string
     */
    ...
    protected function generateTag(Tag $tag, &$html, Tag $openingTag = null, array $openTags = [])
    {
        ...
        switch ($tag->name) {
            ...
            case self::TAG_NAME_URL:
                if ($tag->opening) {
                    if ($tag->property) {
                        $code = '<a target="blank_" href="' . $tag->property . '">';
                    } else {
                        $code = '<a target="blank_" href="';
                    }
                } else {
                    if ($openingTag->property) {
                        $code = '</a>';
