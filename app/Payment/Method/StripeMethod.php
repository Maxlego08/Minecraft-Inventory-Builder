...
class StripeMethod extends PaymentMethod
{
    ...
    public function startPayment(User $user, User\UserPaymentInfo $paymentInfo, Payment $payment, float $price, Gift $gift = null): mixed
    {
        ...
        $logo = match ($type) {
            Payment::TYPE_RESOURCE => $payment->resource->getIconPath(),
            Payment::TYPE_ACCOUNT_UPGRADE => "todo",
        };
        ...
        return view('resources.purchase.stripe', [
            'pk_live' => $pk_live,
     ...
     * @throws ApiErrorException
     */
    private function createSession(User\UserPaymentInfo $paymentInfo, User $user, $price, string $name, string $description, string $content, string $paymentId, string $logo): Session
    {
    ...
    public function process(Request $request, ?string $paymentId): mixed
    {
        ...
        if ($endpointSecret === null)
            return json_encode([
                'status' => 'error',
                'message' => 'User endpoint secrset not found',
            ]);
        ...
        switch ($event->type) {
            case 'checkout.session.completed':
                if ($payment->isPaid())
                    return json_encode([
                        'status' => 'error',
                        'message' => 'The payment is already validated.',
                    ]);
                if ($payment->isDispute())
                    return json_encode([
                        'status' => 'error',
                        'message' => 'The payment is in dispute.',
                    ]);
            ...
            case 'charge.refunded':
                ...
                if (!$payment->isPaid())
                    return json_encode([
                        'status' => 'error',
                        'message' => 'The payment is not validated.',
                    ]);
            ...
            default:
                return json_encode([
                    'status' => 'error',
                    'message' => 'Method ' . $event->type . ' not found',
                ]);
        }
    }
